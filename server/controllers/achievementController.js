...
const {
  buildOwnedObject,
  buildUnownedObject,
} = require("../util/achievementFormatting");

// @desc Get Achievements
// @route GET /api/achievement/?selection=[owned/unowned]
// @access private
const getAchievements = asyncHandler(async (req, res) => {
  ...
  // build achievement objects based on ownership
    ...
    for (let i = 0; i < allAchievements.length; i++) {
      ...
      if (
        achievements.some((achievement) => {
          return achievement.achievementSpecId.equals(allAchievements[i]._id);
        })
      )
        continue;
